...
public class OrderCreateService {
    ...
    /**
     * 생성자를 통해 주문을 생성하여 주문상태를 ORDERD로 표시,
     * 유저와 주문리스트 번호 가지고 저장,
     * 상품 재고 부족시 상품 소진 알리는 익셉션 발생
     **/
    public void create(Long userId, Long productId, int quantity, Address address, String paymentMethod) {
        User user = userRepository.validateUserId(userId);
        OrderList orderList = findAvailableOrderList(userId, address, paymentMethod);
     ...
     * 이동후 생성후 다음 로직 진행
     **/
    private OrderList findAvailableOrderList(Long userId, Address address, String paymentMethod) {
        return orderListRepository.findByUserIdAndStatus(userId, true).orElseGet(() ->
                orderListAddService.add(userId, address, paymentMethod)
        );
    }
